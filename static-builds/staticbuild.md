...
git clone https://github.com//RPG-Core
mkdir -p release
cd RPG-Core/depends
...
cp -f rpg-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/rpg-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer rpg-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer rpg-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
#transfer rpg-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/rpg-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer rpg-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
