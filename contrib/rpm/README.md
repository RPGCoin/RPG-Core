...
The RPM spec file provided here is for RPG-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://rpg.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[RPG git repository](https://github.com/RPGCoin/RPG-Core) but are not part of
...
Currently only `Source10` is used, for the example `rpg.conf` file.
...
in the rpg git repository. At present only an SVG version of the RPG
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the rpg
...
needed when 0.12.1 is released, a proper fix is already in the RPG git
master branch.
...
idea to build RPG Core against a static version of that library compiled
...
The `rpg.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `rpg-server` package. At this point, most RPM based
...
The `rpg-server` package should have SELinux support. How to properly do
...
RPG-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/rpg/). He has been
packaging RPG for Fedora at least since 2012.
...
4. I renamed the `rpg` package that contains the Qt GUI to `rpg-core` as
that appears to be how the general population refers to it, in contrast to
`rpg-xt` or `rpg-classic`. I wanted to make sure the general population
