...
`test/functional/test_framework/test_framework.py:RPGTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/rpg_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_rpgd = config["components"].getboolean("ENABLE_RPGD")
    ...
    if not (enable_wallet and enable_cli and enable_rpgd):
        print("No functional tests to run. Wallet, utils, and rpgd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if rpgd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "rpgd"]) is not None:
            print("%sWARNING!%s There is already a rpgd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "RPGD" not in os.environ:
        os.environ["RPGD"] = build_dir + '/src/rpgd' + exeext
        os.environ["RPGCLI"] = build_dir + '/src/rpg-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `rpg-cli help` (`rpc_interface.txt`).
