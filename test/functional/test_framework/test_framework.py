...
class RPGTestFramework():
    """Base class for a rpg test script.

    Individual rpg test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave rpgds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop rpgds after the test execution")
        parser.add_option("--srcdir", dest="srcdir",
                          default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing rpgd/rpg-cli (default: %default)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: rpgds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a rpgd test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple rpgd test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'rpgd exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "rpgd should have exited with an error"
                else:
                    assert_msg = "rpgd should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as rpgd's debug.log with microprecision (so log files can be concatenated and sorted)
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("RPGRPC")
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            # Create cache directories, run rpgds:
            for i in range(MAX_NODES):
                datadir = initialize_datadir(self.options.cachedir, i)
                args = [os.getenv("RPGD", "rpgd"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0"]
                if i > 0:
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i)  # Overwrite port/rpcport in rpg.conf
...
class ComparisonTestFramework(RPGTestFramework):
    """Test framework for doing p2p comparison testing

    Sets up some rpgd binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("RPGD", "rpgd"),
                          help="rpgd binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("RPGD", "rpgd"),
                          help="rpgd binary to use for reference nodes (if any)")
