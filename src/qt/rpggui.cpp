...
const std::string RPGGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString RPGGUI::DEFAULT_WALLET = "~Default";

RPGGUI::RPGGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void RPGGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a RPG address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and rpg: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to RPG addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your RPG addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified RPG addresses"));
    ...
    openAction->setStatusTip(tr("Open a rpg: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible RPG command-line options").arg(tr(PACKAGE_NAME)));
...
void RPGGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/rpgcointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* rpgLabelLayout = new QVBoxLayout(toolbarWidget);
        rpgLabelLayout->addWidget(label);
        rpgLabelLayout->addWidget(toolbar);
        rpgLabelLayout->setDirection(QBoxLayout::TopToBottom);
        rpgLabelLayout->addStretch(1);
...
/*
        ...
        labelCurrentMarket->setText(tr("RPGcoin Market Price"));
        ...
        labelBtcRPG->setText("BTC / RPG");
        labelBtcRPG->setContentsMargins(15,0,0,0);
        labelBtcRPG->setFixedHeight(75);
        labelBtcRPG->setAlignment(Qt::AlignVCenter);
        labelBtcRPG->setStyleSheet(STRING_LABEL_COLOR);
        labelBtcRPG->setFont(currentMarketFont);
        ...
        priceLayout->addWidget(labelBtcRPG, 0 , Qt::AlignVCenter | Qt::AlignLeft);
...
void RPGGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool RPGGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool RPGGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void RPGGUI::setWalletActionsEnabled(bool enabled)
{
...
void RPGGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void RPGGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void RPGGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void RPGGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void RPGGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void RPGGUI::openClicked()
{
...
void RPGGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void RPGGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void RPGGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void RPGGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void RPGGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void RPGGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** RPG START */
void RPGGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void RPGGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void RPGGUI::gotoManageAssetsPage()
{
...
void RPGGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to RPG network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void RPGGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void RPGGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void RPGGUI::updateHeadersSyncProgressLabel()
{
...
void RPGGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void RPGGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("RPG"); // default title
...
void RPGGUI::changeEvent(QEvent *e)
{
...
void RPGGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void RPGGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void RPGGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "RPG")
        msg += tr("Amount: %1\n").arg(RPGUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(RPGUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void RPGGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void RPGGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool RPGGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool RPGGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void RPGGUI::setHDStatus(int hdEnabled)
{
...
void RPGGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void RPGGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void RPGGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void RPGGUI::detectShutdown()
{
...
void RPGGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void RPGGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void RPGGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(RPGGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void RPGGUI::subscribeToCoreSignals()
{
...
void RPGGUI::unsubscribeFromCoreSignals()
{
...
void RPGGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<RPGUnits::Unit> units = RPGUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const RPGUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(RPGUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (RPGUnits::Unit u : RPGUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(RPGUnits::name(u)), this);
...
{
    setText(RPGUnits::name(newUnits));
}
...
void RPGGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=RPGBTC"));
