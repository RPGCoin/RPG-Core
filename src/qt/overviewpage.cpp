...
public:
    explicit TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(RPGUnits::RPG),
...
//    ui->verticalLayout_3->setSpacing(10);
    ...
    ui->rpgBalancesLabel->setStyleSheet(COLOR_LABEL_STRING);
    ...
    ui->rpgBalancesLabel->setFont(GUIUtil::getTopLabelFont());
    ...
    /** Setup the RPG Balance Colors for darkmode */
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(RPGUnits::formatWithUnit(unit, balance, false, RPGUnits::separatorAlways));
    ui->labelUnconfirmed->setText(RPGUnits::formatWithUnit(unit, unconfirmedBalance, false, RPGUnits::separatorAlways));
    ui->labelImmature->setText(RPGUnits::formatWithUnit(unit, immatureBalance, false, RPGUnits::separatorAlways));
    ui->labelTotal->setText(RPGUnits::formatWithUnit(unit, balance + unconfirmedBalance + immatureBalance, false, RPGUnits::separatorAlways));
    ui->labelWatchAvailable->setText(RPGUnits::formatWithUnit(unit, watchOnlyBalance, false, RPGUnits::separatorAlways));
    ui->labelWatchPending->setText(RPGUnits::formatWithUnit(unit, watchUnconfBalance, false, RPGUnits::separatorAlways));
    ui->labelWatchImmature->setText(RPGUnits::formatWithUnit(unit, watchImmatureBalance, false, RPGUnits::separatorAlways));
    ui->labelWatchTotal->setText(RPGUnits::formatWithUnit(unit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance, false, RPGUnits::separatorAlways));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("RPG")
    updateDisplayUnit();
}
...
void OverviewPage::showAssets()
{
    if (AreAssetsDeployed()) {
        ...
        // This keeps the RPG balance grid from expanding and looking terrible when asset balance is hidden
