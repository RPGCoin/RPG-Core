...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** RPG START */
    case OP_RPG_ASSET              : return "OP_RPG_ASSET";
    /** RPG END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_RPG_ASSET) { // OP_RPG_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == RPG_R) { // Check to see if RPG starts at 27 ( this->size() < 105)
                if ((*this)[28] == RPG_V)
                    if ((*this)[29] == RPG_N)
                        index = 30;
            } else {
                if ((*this)[28] == RPG_R) // Check to see if RPG starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == RPG_V)
                        if ((*this)[30] == RPG_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == RPG_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == RPG_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == RPG_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == RPG_R) {
