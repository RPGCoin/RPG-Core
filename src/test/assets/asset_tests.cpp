...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of RPGCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("RPG", type));
        BOOST_CHECK(!IsAssetNameValid("RPG", type));
        BOOST_CHECK(!IsAssetNameValid("RPGCOIN", type));

        //- Versions of RPGCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("RPG.COIN", type));
        BOOST_CHECK(IsAssetNameValid("RPG_COIN", type));
        BOOST_CHECK(IsAssetNameValid("RPGSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERRPG", type));
        BOOST_CHECK(IsAssetNameValid("RPGSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERPG", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_RPGS", type));
        BOOST_CHECK(IsAssetNameValid("SERPGOT", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("RPG", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
