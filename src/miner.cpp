...
void static RPGMiner(const CChainParams& chainparams)
{
    LogPrintf("RPGMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rpg-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("RPGMiner -- Wallet not available\n");
    }
        ...
        while (true) {
            ...
            if (chainparams.MiningRequiresPeers()) {
                ...
                do {
                    ...
                    LogPrintf("RPGMiner -- waiting for connections and blocks downloaded\n");
                    MilliSleep(1000);
                } while (true);
            }
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("RPGMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("RPGMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("RPGMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("RPGMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateRPGs(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&RPGMiner, boost::cref(chainparams)));
    }
